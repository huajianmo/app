...
# Maps MIME base types (no codec suffix) to the correct file extension for
# content actually delivered by the SABR server.  When the server advertises a
# format via FORMAT_INITIALIZATION_METADATA, the mimeType field tells us the
# true container — which may differ from what the stream manifest implied.
_SABR_MIME_TO_EXT: Dict[str, str] = {
    'audio/aac': '.aac',   # ADTS AAC (HLS-style, e.g. Apple streaming)
    'audio/mp4': '.m4a',   # Fragmented MP4 audio
    'audio/webm': '.webm',
    'video/mp4': '.mp4',
    'video/webm': '.webm',
}
...
class Stream:
    ...
    def download(
        ...
        sabr_stream: Optional[ServerAbrStream] = None
        with open(file_path, "wb") as fh:
            try:
                if not self.is_sabr:
                    ...
                    sabr_stream = ServerAbrStream(stream=self, write_chunk=write_chunk, monostate=self._monostate)
                    sabr_stream.start()
        ...
        # The file handle is closed here, so it is safe to rename on all platforms.
        # When the SABR server delivers ADTS AAC (e.g. for HLS-backed audio tracks)
        # the FORMAT_INITIALIZATION_METADATA mimeType will be "audio/aac" rather than
        # "audio/mp4", and saving the file with a .mp4 / .m4a extension produces a
        # container/content mismatch.  Rename to the correct extension so tools like
        # ffprobe, VLC, and ffmpeg can identify the format without probing the content.
        if sabr_stream is not None and sabr_stream.sabr_mime_type:
            file_path = self._correct_sabr_extension(file_path, sabr_stream.sabr_mime_type)

        self.on_complete(file_path)
        return file_path

    def _correct_sabr_extension(self, file_path: str, sabr_mime_type: str) -> str:
        """Rename *file_path* when the SABR server delivered a format whose correct
        extension differs from the one already on the file.

        For example, YouTube sometimes delivers ADTS AAC audio (``audio/aac``) for
        streams whose manifest says ``audio/mp4``.  Without this correction the file
        would be saved as ``.m4a`` or ``.mp4`` even though the bytes are raw ADTS
        AAC — a mismatch that confuses ffmpeg, ffprobe, and media players.
        """
        mime_base = sabr_mime_type.split(';')[0].strip().lower()
        target_ext = _SABR_MIME_TO_EXT.get(mime_base)
        ...
        new_path = os.path.splitext(file_path)[0] + target_ext
        logger.warning(
            "SABR delivered '%s' but file was saved as '%s' — renaming to '%s'",
            mime_base, current_ext or '(no ext)', os.path.basename(new_path),
        )
