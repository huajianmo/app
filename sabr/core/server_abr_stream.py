...
class ServerAbrStream:
    def __init__(self, stream, write_chunk: Callable, monostate: Monostate):
        ...
        self.RELOAD = False
        self.maximum_reload_attempt = _MAX_RELOAD_ATTEMPTS
    ...
    def start(self):
        ...
        if audio_format:
            self._audio_format_variants = _build_audio_format_variants(
                self.stream.itag,
                int(self.stream.last_Modified),
                self.stream.xtags,
            )
        ...
        client_abr_state = {
            ...
            'lastManualSelectedResolution': int(self.stream.resolution.replace('p', '')) if video_format else 0,
            'stickyResolution': int(self.stream.resolution.replace('p', '')) if video_format else 0,
            'playerTimeMs': 0,
            'visibility': 0,
        ...
        while client_abr_state['playerTimeMs'] < self.totalDurationMs:
            ...
            if data.get("sabr_error"):
                sabr_error = data["sabr_error"]
                attempts_used = _MAX_RELOAD_ATTEMPTS - self.maximum_reload_attempt + 1
                logger.warning(
                    "SABR Protocol Error: %s. Retrying (%d/%d)...",
                    sabr_error, attempts_used, _MAX_RELOAD_ATTEMPTS,
                )
                if sabr_error.type == 'sabr.no_audio_selected' and self._audio_format_variants:
                    audio_format = self._handle_no_audio_selected(audio_format)
                self.reload()
    ...
    def _handle_no_audio_selected(self, current_audio_format):
        """Advance to the next audio format variant and reset initialized formats."""
        self._no_audio_attempt_index += 1
        if self._no_audio_attempt_index < len(self._audio_format_variants):
            next_variant = self._audio_format_variants[self._no_audio_attempt_index]
            logger.debug(
                "Retrying audio with format variant %d/%d: itag=%s lastModified=%s xtags=%s",
                self._no_audio_attempt_index,
                len(self._audio_format_variants) - 1,
                next_variant['itag'],
                next_variant['lastModified'],
                next_variant['xtags'],
            )
            self.initialized_formats.clear()
            self.formats_by_key.clear()
            return [next_variant]
        return current_audio_format
